...
const FEELING_LABELS = ["", "Veldig dårlig", "Dårlig", "Nøytral", "Bra", "Veldig bra"];
...
export default function TrainingLog({ weeks, saveWeeks }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {["form", "history"].map(t => (
          <button key={t} className={`btn ${tab === t ? "btn-primary" : ""}`} onClick={() => setTab(t)}>
            {t === "form" ? (editIndex !== null ? "✎ REDIGERER" : "+ NY UKE") : `HISTORIKK (${weeks.length})`}
          </button>
        ))}
      </div>
      ...
      {tab === "form" && (
        <div className="card">
          <div className="section-label">{editIndex !== null ? `Redigerer uke ${weeks[editIndex]?.weekStart}` : "Logg ny uke"}</div>

          <div className="grid-2" style={{ marginBottom: 20 }}>
            <div><label>Uke starter</label><input type="date" value={form.weekStart} onChange={e => hc("weekStart", e.target.value)} /></div>
            <div><label>Nøkkeløkt</label><input placeholder="f.eks. 2×20 min IntEnd" value={form.keyWorkout} onChange={e => hc("keyWorkout", e.target.value)} /></div>
          </div>
          ...
          <div className="grid-3" style={{ marginBottom: 20 }}>
            ...
            <div><label>% Easy (sone 1–2)</label><input type="number" placeholder="80" value={form.intensityDistribution.easy} onChange={e => hn("intensityDistribution", "easy", e.target.value)} /></div>
          ...
          <div className="grid-2" style={{ marginBottom: 20 }}>
            ...
            <div><label>Snitt søvn (timer)</label><input type="number" placeholder="7.5" step="0.1" value={form.sleepHours} onChange={e => hc("sleepHours", e.target.value)} /></div>
          </div>

          <div className="section-label">Subjektivt</div>
          <div style={{ marginBottom: 16 }}>
            <label>Akilles – {ACHILLES_LABELS[form.achilles]}</label>
          ...
          <div style={{ marginBottom: 16 }}>
            <label>Form – {FEELING_LABELS[form.feeling]}</label>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 6 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <textarea rows={3} placeholder="Akilles, justeringer, reiser, vær, sykdom..." value={form.comments} onChange={e => hc("comments", e.target.value)} />
          </div>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            {saved && <span style={{ fontSize: 11, color: "var(--green)", letterSpacing: "0.08em" }}>✓ Lagret</span>}
          </div>
        </div>
      )}
      ...
      {tab === "history" && (
        <div>
          {weeks.length === 0 && (
            <div style={{ textAlign: "center", color: "var(--text3)", padding: "60px 0", fontSize: 13 }}>Ingen uker logget ennå</div>
          )}
          {weeks.map((w, i) => (
            <div className="card" key={i} style={{ marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
                  <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 18, fontWeight: 700, letterSpacing: "0.05em" }}>
                    {w.weekStart ? new Date(w.weekStart).toLocaleDateString("nb-NO", { day: "numeric", month: "long", year: "numeric" }) : "—"}
                  </div>
                  {w.keyWorkout && <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 3 }}>◈ {w.keyWorkout}</div>}
                </div>
              ...
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 10 }}>
                {[
                  ...
                  [w.sleepHours && w.sleepHours + "t", "søvn", "var(--text3)"],
